...
// The "new" side of the cross-version test pair: this module declares
// app.cash.zipline.bridge.test.WirePayload with its post-rename field names plus @HostName, and its
// test loads the guest bundle built by :zipline-bridge-skew-old (and vice versa there). See
...
val crossGuestDir = project(":zipline-bridge-skew-old").layout.buildDirectory.dir("generated/guest")
...
val bridgeSoFile = layout.buildDirectory.file(
  if (isMac) "generated/bridge/libskewnew.dylib" else "generated/bridge/libskewnew.so",
)
...
tasks {
  ...
  // jsMain edit plus `./gradlew :zipline-bridge-skew-new:compileDevelopmentExecutableKotlinJs`; a
  ...
  val generateGuestSource = register("generateGuestSource") {
    ...
    doLast {
      ...
      sb.appendLine()
      sb.appendLine("object GeneratedSkewNewGuest {")
      ...
      sb.appendLine("  )")
      sb.appendLine("}")
      val outFile = generatedGuestDir.get().asFile.resolve("GeneratedSkewNewGuest.kt")
  ...
  kotlin.targets.configureEach {
    compilations.matching { it.name == "test" }.configureEach {
      compileTaskProvider.configure {
        ...
        dependsOn(":zipline-bridge-skew-old:generateGuestSource")
      }
    }
  }
}
